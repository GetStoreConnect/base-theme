...
function init(node) {
  [...node.querySelectorAll('[data-delivery-options]')].forEach(node => DeliveryOptionsSelector(node));
}
...
const DeliveryOptionsSelector = (function (node) {
  ...
  function allValid() {
    return [...node.querySelectorAll('[data-delivery-options]')].every((node) => node.hasAttribute('data-valid'))
  }
  ...
  async function fetchDeliveryOptions() {
    ...
    const response = await fetch(`/async/delivery_options?${queryParam()}&line_item_id=${lineItemId}`)
