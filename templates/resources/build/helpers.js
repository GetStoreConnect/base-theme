...
function notify({ type, color }) {
  return {
    ...
    setup(build) {
      build.onStart(() => {
        console.log(colors[color](`[esbuild theme ${type}] building...`))
      })
      build.onEnd((result) => {
        if (result.errors.length > 0) {
          console.error(
            colors.red(`theme ${type} build failed with ${result.errors.length} errors`)
          )
        }
        console.log(colors[color](`[esbuild theme ${type}] ...done`))
      })
    },
  }
}
