(() => {
  ...
  function init(node) {
    ;
    [...node.querySelectorAll("[data-booking]")].forEach((node2) => BookingSelector(node2));
  }
  var BookingSelector = function(node) {
    ...
    function start() {
      ...
      if (!srtDate || !endDate) {
        srtDate = srtDate || (endDate ? formatDate(Date.parse(endDate) - 7 * 24 * 60 * 60 * 1e3) : formatDate(Date.now()));
        endDate = endDate || formatDate(Date.parse(srtDate) + 7 * 24 * 60 * 60 * 1e3);
      }
      if (srtDate > endDate) {
        const start2 = Date.parse(srtDate);
        endDate = formatDate(start2 + 7 * 24 * 60 * 60 * 1e3);
      }
    ...
    function handleTimeslotSelection() {
      ...
      [...node.querySelectorAll("[data-booking-timeslot]")].map((timeslot) => {
        timeslot.addEventListener("change", (event) => {
          ...
          qtyPickers.forEach((picker2) => {
            const input = picker2.querySelector('input[name="quantity"]');
    ...
    function highlightDays(date, direction, preload = false) {
      ...
      fetchAvailabilities(endpoint, "application/json").then((days) => {
        if (!preload) {
    ...
    function filterAvailabilities(startDate, endDate, locationId) {
      ...
      const loader = loader_default({ target: slots, contextual: true });
      loader.on();
      ...
      slots.scrollTop = 0;
      fetchAvailabilities(endpoint).then((text) => {
        loader.off();
    ...
    function returnEndPoint({
      ...
      if (!availabilitiesUrl) availabilitiesUrl = "/availabilities";
      let url = `${availabilitiesUrl}?product_id=${productId}&start_date=${startDate}&end_date=${endDate}`;
      if (locationId && locationId != "on") url = `${url}&product_bookable_location_id=${locationId}`;
      if (partial) url = `${url}&partial=${partial}`;
      if (highlights) url = `${url}&highlight=true`;
      return url;
    }
    function returnRangeAsString(startDate, endDate) {
      ...
      return dateTime.format(new Date(startDate)) + " - " + dateTime.format(new Date(endDate));
    }
    ...
    function handleClosestLinks() {
      ...
      jumpLinks.forEach(
        (trigger2) => trigger2.addEventListener("click", (event) => {
          ...
          if (direction == "back") {
            ...
            start2 = end - 7 * 24 * 60 * 60 * 1e3;
          } else {
            start2 = Date.parse(event.target.getAttribute("data-booking-jump"));
            end = start2 + 7 * 24 * 60 * 60 * 1e3;
          }
          picker.setDateRange(start2, end, true);
          setHighlightDays();
        })
      );
      if (autoJump) {
...
litepicker/dist/litepicker.umd.js:
   ...
   *)
*/
//# sourceMappingURL=booking.CSEJYEMJ.js.map
