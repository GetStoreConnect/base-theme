(() => {
  // src/scripts/theme/bundle-filters.js
  ...
  function filterProductList() {
    ...
    [...container.querySelectorAll("[data-product-option-sfid]")].map((product) => {
      ...
      } else if ((selectedBrands.includes(brandId) || selectedBrands.length === 0) && (selectedCategories.filter((value) => categoryIds.includes(value)).length > 0 || selectedCategories.length === 0)) {
        product.classList.remove("is-hidden");
      } else {
  ...
  function setup(container, bundleId) {
    ...
    initVariants();
    featureOptions.filter((option) => option.dataset.selected).forEach((option) => {
    ...
    function addOrUpdateProductOption(productId, featureId, qty, isSingleOption, onSuccessCallback) {
      ...
      fetchWithResponseHandler(
        store_path_url_default(`/cpq/bundles/${bundleId}/product_options/${productId}`),
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded"
      ...
      ).then(() => onSuccessCallback()).catch((error) => console.error("Fetch error:", error));
    }
    ...
    function returnHeadline(optionCard, productCard) {
      return optionCard.querySelector("[data-option-card-subset]") ? productCard.querySelector("[data-single-option-heading]").innerText : optionCard.querySelector("[data-single-option-heading]").innerText;
    }
    function chooseProductVariant(e) {
      ...
      fetchWithResponseHandler(store_path_url_default(`/cpq/bundles/${bundleId}/product_options/${id}`), {
        method: "PATCH",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded"
        },
    ...
    function returnSelectedVariantOptions(variantOptions) {
      return variantOptions.filter((opt) => opt.checked || opt.selected).sort((a, b) => a.dataset.type < b.dataset.type ? -1 : 1);
    }
    ...
    function disableOrEnableVariantOptions(option, variantOptions) {
      ...
      document.querySelectorAll("[data-variant-option]").forEach((group) => {
        ...
        optionsInGroup.forEach((option2) => {
          option2.disabled = !JSON.parse(allowedOptions)[groupType].includes(option2.value);
        });
      });
    }
    ...
    function showOrHideVariantContent(selectedVariant) {
      document.querySelectorAll("[data-variant-id]").forEach((node) => {
        ...
        const isSelected = isMaster ? selectedVariant === void 0 : node.dataset.variantId === selectedVariant;
    ...
    function initOptionCard(onChangeCallback) {
      ...
      inputs.forEach((input) => {
        input.addEventListener("change", (event) => {
          ...
          if (inputElement) {
            qtyInputs.forEach((input2) => input2.value = 0);
      ...
      function choose(event, onChangeCallback2) {
...
//# sourceMappingURL=bundle.FBK7XZ7W.js.map
