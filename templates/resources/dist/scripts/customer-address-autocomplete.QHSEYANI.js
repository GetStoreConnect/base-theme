(() => {
  ...
  // src/scripts/packs/customer-address-autocomplete.js
  onDomChange(init);
  function init(node) {
    ...
    if (autocomplete) {
      ...
      autocomplete.addEventListener("input", function(event) {
        fetch(store_path_url_default("/checkout/valid_addresses"), {
          method: "GET",
          headers: {
          ...
          data2.forEach((address) => {
            ...
            option.value = address["address_1"];
...
//# sourceMappingURL=customer-address-autocomplete.QHSEYANI.js.map
