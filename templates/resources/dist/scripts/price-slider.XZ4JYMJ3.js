(() => {
  // src/scripts/theme/utils/rangeslider.js
  var IonRangeSlider = function(element, initOptions = {}) {
    let input = element;
    ...
    for (let prop in config_from_data) {
      if (config_from_data.hasOwnProperty(prop)) {
        if (config_from_data[prop] === void 0 || config_from_data[prop] === "") {
    ...
    const base_html = '<span class="irs"><span class="irs-line" tabindex="0"></span><span class="irs-min">0</span><span class="irs-max">1</span><span class="irs-from">0</span><span class="irs-to">0</span><span class="irs-single">0</span></span><span class="irs-grid">1</span>';
    const single_html = '<span class="irs-bar irs-bar--single"></span><span class="irs-shadow shadow-single"></span><span class="irs-handle single"><i></i><i></i><i></i></span>';
    const double_html = '<span class="irs-bar"></span><span class="irs-shadow shadow-from"></span><span class="irs-shadow shadow-to"></span><span class="irs-handle from"><i></i><i></i><i></i></span><span class="irs-handle to"><i></i><i></i><i></i></span>';
    const disable_html = '<span class="irs-disable-mask"></span>';
    const init2 = function(is_update2) {
      no_diapason = false;
    ...
    const setTopHandler = function() {
      const min = options.min, max = options.max, from = options.from, to = options.to;
      if (from > min && to === max) {
    ...
    const changeLevel = function(target2) {
      switch (target2) {
        case "single":
    ...
    const pointerFocus = function(e) {
      if (!target) {
        ...
        pointerClick("single", { preventDefault: function() {
        }, pageX: x });
      } else {
        cache.line.focus();
      }
    ...
    const calc = function(update2) {
      if (!options) {
        return;
      }
      calc_count++;
      if (calc_count === 10 || update2) {
        calc_count = 0;
      ...
      switch (target) {
        case "base":
          const w = (options.max - options.min) / 100, f = (result.from - options.min) / w, t = (result.to - options.min) / w;
        ...
        case "both":
          ...
          handle_x = toFixed(handle_x + coords.p_handle * 1e-3);
        ...
        case "both_one":
          ...
          const real_x = convertToRealPercent(handle_x), from = result.from_percent, to = result.to_percent, full = to - from, half = full / 2;
          let new_from = real_x - half, new_to = real_x + half;
          if (new_from < 0) {
            new_from = 0;
    ...
    const drawLabels = function() {
      ...
      const values_num = options.values.length, p_values = options.p_values;
      ...
      if (options.type === "single") {
        ...
        calcLabels();
        const min = Math.min(labels.p_single_left, labels.p_from_left), single_left = labels.p_single_left + labels.p_single_fake, to_left = labels.p_to_left + labels.p_to_fake;
    ...
    const drawShadow = function() {
      const o = options, c = cache, is_from_min = typeof o.from_min === "number" && !isNaN(o.from_min), is_from_max = typeof o.from_max === "number" && !isNaN(o.from_max), is_to_min = typeof o.to_min === "number" && !isNaN(o.to_min), is_to_max = typeof o.to_max === "number" && !isNaN(o.to_max);
    ...
    const convertToPercent = function(value, no_min) {
      let diapason = options.max - options.min, one_percent = diapason / 100, val2, percent;
      if (!diapason) {
        no_diapason = true;
        return 0;
      }
      if (no_min) {
        ...
        val2 = value - options.min;
      }
      percent = val2 / one_percent;
      return toFixed(percent);
    };
    const convertToValue = function(percent) {
      let min = options.min, max = options.max, min_decimals = min.toString().split(".")[1], max_decimals = max.toString().split(".")[1], min_length, max_length, avg_decimals = 0, abs = 0;
      if (percent === 0) {
        return options.min;
      }
      if (percent === 100) {
      ...
      let number = (max - min) / 100 * percent + min, string = options.step.toString().split(".")[1], result2;
      if (string) {
      ...
      string ? result2 = +number.toFixed(string.length) : result2 = toFixed(number);
      if (result2 < options.min) {
        result2 = options.min;
      } else if (result2 > options.max) {
        result2 = options.max;
      }
      return result2;
    };
    ...
    const validate = function() {
      let o = options, r = result, v = o.values, vl = v.length, value, i;
    ...
    const appendGrid = function() {
      ...
      let o = options, i, z, total = o.max - o.min, big_num = o.grid_num, big_p = 0, big_w = 0, small_max = 4, local_small_max, small_p, small_w = 0, result2, html = "";
      calcGridMargin();
      if (o.grid_snap) {
      ...
      for (i = 0; i < big_num + 1; i++) {
        ...
        result2 = convertToValue(big_w);
        if (o.values.length) {
          result2 = o.p_values[result2];
        } else {
          result2 = _prettify(result2);
        }
        html += '<span class="irs-grid-text js-grid-text-' + i + '" style="left: ' + big_w + '%">' + result2 + "</span>";
      }
    ...
    const calcGridLabels = function() {
      const start = [], finish = [], num = coords.big_num;
      for (let i = 0; i < num; i++) {
  ...
  // src/scripts/packs/price-slider.js
  onDomChange(init);
  function init(node) {
...
//# sourceMappingURL=price-slider.XZ4JYMJ3.js.map
