(() => {
  ...
  // src/scripts/packs/gateways/tyro.js
  onDomChange((node) => {
  ...
  var tyro;
  var tyroForm;
  function initTyro({ form: form2 }) {
    init(form2, tyroSubmitPayment);
    setPayButton(false);
    fetchWithResponseHandler(callbackUrl(), {
      ...
      initializeTyro(form2);
    });
  }
  async function initializeTyro(form2) {
    tyro = Tyro({
      liveMode: form2.dataset.apiMode === "production"
    });
    if (form2.dataset.error) {
      showError(form2.dataset.error);
      return;
    }
    try {
      setPayButton(false);
      ...
      const inlineFormId = `${form2.id}[data-ref="inline-form"]`;
  ...
  async function tyroWalletPaymentComplete(paymentType, error) {
    ...
    try {
      ...
      handleFailedPayment({ message: error2.errorMessage, error: error2 });
    }
  }
...
//# sourceMappingURL=tyro.FFRV3UII.js.map
