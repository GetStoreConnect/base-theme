(() => {
  ...
  // src/scripts/packs/gateways/cyber-source.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider="CyberSource"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initCyberSource({ form: form2, providerId: providerId2 });
      }
    });
  });
  var microformInstance = null;
  async function initCyberSource({ form: form2, providerId: providerId2 }) {
    init(form2, submitPaymentForm);
  ...
  async function submitPaymentForm(form2) {
    const tokenOptions = {
    ...
    await microformInstance.createToken(tokenOptions, (err, token) => {
      ...
      const payload = {
        payment: {
          provider_id: form2.dataset.providerId,
          method: form2.dataset.provider
        },
        payment_source: {
          tok_id: token,
...
//# sourceMappingURL=cyber-source.2OX4UOI4.js.map
