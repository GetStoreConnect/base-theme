(() => {
  ...
  // src/scripts/packs/gateways/authorize-net.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider*="AuthorizeNet"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initAuthorizeNet({ form: form2 });
      }
    });
  });
  function initAuthorizeNet({ form: form2 }) {
    const zipCode = form2.dataset.zipCode;
    init(form2, createToken);
    const authData = {
      apiLoginID: apiKey(),
      clientKey: form2.dataset.publicKey
    };
...
//# sourceMappingURL=authorize-net.6CK6735X.js.map
