(() => {
  ...
  onDomChange((node) => {
    ...
    forms.forEach((form) => {
      ...
      initStripe({ form, providerId: providerId2 });
    });
  });
  function initStripe({ form, providerId: providerId2 }) {
    let dedicatedCartProductId2 = form.dataset.cartProductId;
    init("Stripe", providerId2, stripeCreateToken, null, dedicatedCartProductId2);
    let cardNumberElement;
    let stripe;
    let elements;
    function paymentsUrl() {
      return form.dataset.paymentsUrl;
    }
    function intentsUrl() {
    ...
    if (window.StoreConnectTestMode === "enabled") {
      window.testStripeExpressCheckout = async ({ dedicatedProductId, shippingRateId }) => {
        ...
        document.querySelectorAll('form[action="/cart"]').forEach((form2) => {
...
//# sourceMappingURL=stripe.E3DNEF3U.js.map
