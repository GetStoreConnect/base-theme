(() => {
  ...
  // src/scripts/packs/gateways/square.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form.sq-payment-form[data-provider="Square"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initSquare({ form: form2, providerId: providerId2 });
      }
    });
  });
  async function initSquare({ form: form2, providerId: providerId2 }) {
    init(form2, createToken);
    ...
    async function verifyBuyer(payments2, token) {
      ...
      const verificationResults = await payments2.verifyBuyer(token, verificationDetails);
    ...
    loadScript({
      ...
      onload: async function() {
        ...
        const locationId = form2.dataset.locationId;
        try {
          ...
          Bugsnag.notify(e);
          const statusContainer = document.getElementById(`SquarePaymentStatus${providerId2}`);
        ...
        async function initializeCard(payments2) {
          const card2 = await payments2.card();
          await card2.attach(`#SquarePaymentFields${providerId2}`);
          return card2;
        }
        function buildPaymentRequest(payments2) {
          return payments2.paymentRequest({
        ...
        async function initializeApplePay(payments2) {
          const paymentRequest = buildPaymentRequest(payments2);
          return payments2.applePay(paymentRequest);
        }
        if (showWallets()) {
          const googlePayButtonId = `SquareGooglePaymentButton${providerId2}`;
          if (document.getElementById(googlePayButtonId)) {
            if (document.querySelector(`#${googlePayButtonId}`)) {
              try {
                ...
                document.getElementById(googlePayButtonId).addEventListener("click", async function(event) {
          ...
          const applePayButton = document.getElementById(`SquareApplePaymentButton${providerId2}`);
          if (applePayButton) {
          ...
          const container = document.getElementById(`SquareWalletsContainer${providerId2}`);
          if (container) {
            container.remove();
          }
        }
...
//# sourceMappingURL=square.WWZMUOIS.js.map
