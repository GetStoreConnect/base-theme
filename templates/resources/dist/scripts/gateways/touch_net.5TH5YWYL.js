(() => {
  ...
  function cacheFormParamsAndOnSubmit(onSubmit) {
    const payload = extractAdditionalFormPayload();
    if (Object.keys(payload).length > 0) {
      fetchWithResponseHandler(store_path_url_default("/cache_form_params"), {
        method: "POST",
  ...
  // src/scripts/packs/gateways/touch_net.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider="TouchNet"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initTouchNet({ form: form2 });
      }
    });
  });
  function initTouchNet({ form: form2 }) {
    init(form2, (form3) => cacheFormParamsAndOnSubmit(() => form3.submit()));
    fetchWithResponseHandler(callbackUrl(), {
      ...
      initializeTouchNetForm({ form: form2, response });
    });
  }
  function initializeTouchNetForm({ form: form2, response }) {
    addFormInput({ form: form2, name: "TICKET", value: response.token.ticket });
    addFormInput({ form: form2, name: "TICKET_NAME", value: response.token.ticket_name });
    addFormInput({ form: form2, name: "UPAY_SITE_ID", value: form2.dataset.upaySiteId });
    setPayButton(true);
  }
})();
//# sourceMappingURL=touch_net.5TH5YWYL.js.map
