(() => {
  ...
  function basicInit(providerName_, providerId_) {
  ...
  onDomChange((node) => {
    ...
    forms.forEach((form) => {
      ...
      initAci({ form, providerId: providerId2 });
    });
  });
  function initAci({ form, providerId: providerId2 }) {
    basicInit("Aci", providerId2);
    form.action = form.dataset.paymentsUrl;
    ...
    addFormInput("cde-Name-0", form.dataset.uniqueId);
    addFormInput("productId", form.dataset.productId);
    addFormInput("paymentAmount", totalPayable());
    addFormInput("postbackUrl", form.dataset.postbackUrl);
    addFormInput("returnUrl", form.dataset.returnUrl);
    addFormInput("errorUrl", form.dataset.cancelUrl);
    addFormInput("cancelUrl", form.dataset.cancelUrl);
    addFormInput("firstName", form.dataset.firstName);
    addFormInput("lastName", form.dataset.lastName);
    addFormInput("email", form.dataset.billingEmail);
  }
})();
//# sourceMappingURL=aci.GSVCUUY3.js.map
