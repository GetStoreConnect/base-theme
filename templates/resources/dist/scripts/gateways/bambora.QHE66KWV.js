(() => {
  ...
  // src/scripts/packs/gateways/bambora.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider="Bambora"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initBambora({ form: form2, providerId: providerId2 });
      }
    });
  });
  function initBambora({ form: form2, providerId: providerId2 }) {
    const cardNumberSelector = `card_number__payment__${providerId2}`;
    const cardVerificationSelector = `card_verification__payment__${providerId2}`;
    const cardExpirySelector = `card_expiry__payment__${providerId2}`;
    var checkout;
    ...
    function hideErrorForId(id) {
      ...
      if (element !== null) {
        const errorElement2 = document.getElementById(`${id}-error`);
    ...
    function showErrorForId(id, message) {
      ...
      if (element !== null) {
        const errorElement2 = document.getElementById(`${id}-error`);
        if (errorElement2 !== null) {
          errorElement2.innerHTML = message;
        }
        const bootStrapParent = document.getElementById(`${id}-bootstrap`);
        if (bootStrapParent !== null) {
...
//# sourceMappingURL=bambora.QHE66KWV.js.map
