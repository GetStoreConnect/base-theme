(() => {
  ...
  onDomChange((node) => {
    ...
    forms.forEach((form) => {
      ...
      initEwayV2({ providerId: providerId2, form });
    });
  });
  function initEwayV2({ providerId: providerId2, form }) {
    init("EwayV2", providerId2, getFormFields);
    function getFormFields() {
      ...
      function resultCallback(result, transactionID, errors) {
        if (result === "Complete") {
          confirmPayment(accessCode, transactionID, providerId2, redirectUrl);
      ...
      if (typeof eCrypt !== "undefined") {
        eCrypt.showModalPayment({ sharedPaymentUrl }, resultCallback);
      } else {
        console.error("eCrypt is not defined.");
      }
    }
    function confirmPayment(accessCode, transactionID, paymentProviderId, redirectUrl) {
...
//# sourceMappingURL=eway-v2.INLJBEEA.js.map
