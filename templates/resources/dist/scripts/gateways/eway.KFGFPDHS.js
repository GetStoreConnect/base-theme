(() => {
  ...
  onDomChange((node) => {
    ...
    forms.forEach((form) => {
      ...
      initEway({ form, providerId: providerId2 });
    });
  });
  function initEway({ form, providerId: providerId2 }) {
    init("Eway", providerId2, getFormFields);
    const publicKey = form.dataset.publicKey;
    function getFormFields(form2) {
      const cardNumberEl = form2.querySelector("[data-encrypt-name='EWAY_CARDNUMBER']");
      const cvnEl = form2.querySelector("[data-encrypt-name='EWAY_CARDCVN']");
      const payload = {
        payment_source: {
...
//# sourceMappingURL=eway.KFGFPDHS.js.map
