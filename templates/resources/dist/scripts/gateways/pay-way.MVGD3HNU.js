(() => {
  ...
  function basicInit(form_) {
    ...
    providerId2 = form.dataset.providerId;
  }
  ...
  // src/scripts/packs/gateways/pay-way.js
  var import_ujs2 = __toESM(require_rails_ujs());
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider="PayWay"]');
    forms.forEach((form2) => {
      const providerId3 = form2.dataset.providerId;
      if (providerId3) {
        initPayWay({ form: form2 });
      }
    });
  });
  function initPayWay({ form: form2 }) {
    init(form2, onSubmit);
    const apiKey = form2.dataset.apiKey;
    const payButton = submitElement();
...
//# sourceMappingURL=pay-way.MVGD3HNU.js.map
