(() => {
  ...
  // src/scripts/packs/gateways/bosch.js
  onDomChange((node) => {
    const forms = node.querySelectorAll('form[data-provider="Bosch"]');
    forms.forEach((form2) => {
      const providerId2 = form2.dataset.providerId;
      if (providerId2) {
        initBosch({ form: form2, providerId: providerId2 });
      }
    });
  });
  async function initBosch({ form: form2, providerId: providerId2 }) {
    basicInit(form2);
    const providerName2 = form2.dataset.provider;
    const button = submitElement();
    try {
      const url = form2.dataset.prepareFormDataUrl;
      if (!url) {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          payment: {
            method: providerName2
          },
          payment_provider_id: providerId2
        })
      });
      const data = await response.json();
      if (response.ok && data.url) {
        button.setAttribute("href", data.url);
        form2.classList.remove("sc-hide");
      } else {
        console.error(data.error || "Error initializing Bosch payment form");
        showError(
          Array.isArray(data.error) ? data.error.join(", ") : data.error || "An error occurred initializing the payment form."
        );
      }
    } catch (error) {
...
//# sourceMappingURL=bosch.Q6NJHTQN.js.map
