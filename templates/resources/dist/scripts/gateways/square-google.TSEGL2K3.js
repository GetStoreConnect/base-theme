(() => {
  ...
  function elementProviderId() {
    if (dedicatedCartProductId) {
      return `${providerId2}Product${dedicatedCartProductId}`;
    }
    return providerId2;
  }
  ...
  function submitElement() {
    return document.getElementById(`${providerName}PaymentButton${providerId2}`);
  }
  ...
  function showError(error, replace = true) {
    ...
    if (errorContainer) {
      ...
      if (providerName) {
        console.warn(
          `Provider '${providerName}' does not have a #${providerName}PaymentError${providerId2} div container`
        );
      } else {
  ...
  function submitData({ payload, handleSuccess }) {
    ...
    payload.payment.provider_id = providerId2;
    payload.payment.method = providerName;
    const customerNotes = document.getElementById(`customer_notes__payment__${providerId2}`);
    if (customerNotes) {
      payload.customer_notes = customerNotes.value;
    }
    const assistedBy = document.getElementById(`assisted_by_user_id__payment__${providerId2}`);
    if (assistedBy) {
  ...
  // src/scripts/packs/gateways/square-google.js
  async function verifyBuyer(payments2, token2, data2) {
    const verificationDetails = {
      amount: totalPayable(),
      billingContact: {
        givenName: data2.firstname,
        familyName: data2.lastname,
        email: data2.email,
        phone: data2.phone,
        addressLines: [data2.billingStreet],
        city: data2.billingCity,
        countryCode: data2.billingCountry
      },
      currencyCode: data2.currency,
      intent: "CHARGE"
    };
    const verificationResults = await payments2.verifyBuyer(token2, verificationDetails);
  ...
  async function preSubmit(_form) {
    try {
      const token2 = await tokenize(card);
      const verificationToken = await verifyBuyer(payments, token2, data);
      const payload = {
        payment_source: {
          locationId,
          tok_id: token2,
  ...
  window.StoreConnect.Gateways.SquareGoogle = function(data2 = {
    providerId,
    billingStreet,
    billingCity,
    postCode,
    ...
    init("SquareGoogle", providerId3, preSubmit);
    const providerId3 = data2.providerId;
    const billingCountry2 = data2.billingCountry;
    const locationId2 = formElement().dataset.locationId;
    document.addEventListener("DOMContentLoaded", async function() {
      ...
      try {
        payments2 = window.Square.payments(applicationId, locationId2);
      } catch (e) {
        Bugsnag.notify(e);
        const statusContainer = document.getElementById(`SquareGooglePaymentStatus${providerId3}`);
        statusContainer.className = "missing-credentials";
        statusContainer.style.visibility = "visible";
        return;
      }
      try {
        const paymentRequest = payments2.paymentRequest({
          countryCode: billingCountry2,
        ...
        const button = `SquareGooglePaymentButton${providerId3}`;
        const paymentMethod = await payments2.googlePay(paymentRequest);
        ...
        document.getElementById(button).addEventListener("click", async function(event) {
          ...
          const verification_token = await verifyBuyer(payments2, token, data2);
          const payload = {
...
//# sourceMappingURL=square-google.TSEGL2K3.js.map
