(() => {
  ...
  function initSecurePay({ form: form2, providerId: providerId2 }) {
    var mySecurePayUI;
    init(form2, function() {
      mySecurePayUI.tokenise();
    });
    const inputStyles = form2.dataset.inputStyles ? JSON.parse(form2.dataset.inputStyles) : defaultInputStyles();
    const scriptId = `SecurePayScript${providerId2}`;
    ...
    loadScript({
      ...
      id: scriptId,
      onload: function() {
        const merchantCode = form2.dataset.merchantCode;
        setPayButton(false);
        ...
        function initStyles() {
          form2.getElementsByClassName("securepay-ui-iframe")[0].classList.add("sc-border-none");
        }
        ...
        mySecurePayUI = new securePayUI.init({
          containerId: `SecurePayContainer${providerId2}`,
          scriptId,
          clientId: apiKey(),
          merchantCode,
          card: {
...
//# sourceMappingURL=secure-pay.KTQSVG3K.js.map
